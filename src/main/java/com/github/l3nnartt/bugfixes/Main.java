...
public class Main {
		...
		try {
			...
			if (showConfirmDialog(String.format(lang.get("installation"), "1.0"))) {
				...
				if (run.exists() && run.isFile()) {
					for (String version : new String[] { "1.16" }) {
						File addonsDir = new File(dir + "addons-" + version);
						addonsDir.mkdirs();
						File mod = new File(addonsDir, "Bugfixes.jar");
						if (!mod.exists()) {
							for (File addons : addonsDir.listFiles()) {
								if (addons.getName().toLowerCase().contains("Bugfixes")) {
									addons.delete();
								}
							}
						}
	...
	private static void initLookAndFeel() {
		try {
			...
			URL url = Main.class.getResource("/assets/minecraft/bugfixes/Logo.png");
	...
	private static void initLanguage() {
		if (Locale.getDefault().toString().toLowerCase().contains("de")) {
			lang.put("installation",
					"Bugfixes v%s kann nun installiert werden!\nMinecraft muss bei der Installation geschlossen sein!");
			lang.put("success", "Bugfixes Installation abgeschlossen!");
			...
			lang.put("installation",
					"¡Se puede instalar el Bugfixes v%s ahora!\n¡Minecraft tiene que estar cerrador para instalarlo!");
			lang.put("success", "La instalación del Bugfixes a terminado");
	...
	private static boolean showConfirmDialog(String msg) {
		return JOptionPane.showConfirmDialog(null, msg, "Bugfixes \u00a9 L3nnart_", 2, 1, icon) == 0;
	}

	private static void showMessageDialog(String msg, int mode) {
		JOptionPane.showMessageDialog(null, msg, "Bugfixes \u00a9 L3nnart_", mode);
	}
}
