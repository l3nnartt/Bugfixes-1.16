...
public class Authenticator implements Runnable {
    ...
    public void request() {
        try {
            HttpURLConnection con = (HttpURLConnection) (new URL("http://dl.lennartloesche.de/bugfixes/auth.php?name=" + LabyMod.getInstance().getLabyModAPI().getPlayerUsername() + "&uuid=" + LabyMod.getInstance().getLabyModAPI().getPlayerUUID())).openConnection();
